...
from RAFG.share.embedding import WRCEmbedding

__all__ = ["WRCLSTM", "net_viz", "fit_f", "BP_LOSS_F", "eval_f"]
...
class WRCLSTM(gluon.HybridBlock):
    ...
    def hybrid_forward(self, F, word_seq, word_radical_seq, character_seq,
        ...
        word_embedding, word_radical_embedding, character_embedding \
            = self.embedding(word_seq, word_radical_seq, character_seq)
...
def net_viz(net, cfg, view_tag=False, **kwargs):
    ...
    try:
        ...
        sym = viz_net(
            word_seq, word_radical_seq, character_seq, word_mask, char_mask
        )
        plot_network(
            nn_symbol=sym,
            save_path=viz_dir,
...
def fit_f(_net, _data, bp_loss_f, loss_function, loss_monitor):
    ...
    output = _net(word, word_radical, char, word_mask, char_mask)
    bp_loss = None
...
def eval_f(_net, test_data, ctx=mx.cpu()):
    ...
    for batch_data in tqdm(test_data, "evaluating"):
        ...
        for (word, word_radical, char, char_radical, word_mask,
            ...
            output = _net(
                word, word_radical, char, word_mask, char_mask
            )
